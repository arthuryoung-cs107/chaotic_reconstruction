...
while(-e "$dr/$ARGV[1].$a") {
    ...
    # given
    # $pov_cmd="nice -n 19 povray $tf -D +O$fn $pov_opts"; ## this nice -n 19 option is making the povray sim into a slow task
    if($opt_r) {
        ...
        $pov_cmd="nice -n -10 povray $tf -D +O$fn $pov_opts";
        # Run POV-Ray locally
        print "Frame $a\n";
