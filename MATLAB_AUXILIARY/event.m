classdef event
    ...
    methods
        function obj = event(dat_dir_name_, exp_name_, dat_name_, relay_id_, event_count_, specs)
            ...
            event_data = struct('int_vec', fread(dat_post,[head(1),1], 'int=>int'), ...
                            'double_vec', fread(dat_post,[head(2),1], 'double=>double'), ...
                            'event_frame_count', fread(dat_post,[specs.beads*specs.Frames,1], 'int=>int'), ...
                            'event_frames', fread(dat_post,[specs.beads,1], 'int=>int'));
            for i = 1:specs.npool
                pool_data.int_data(:,i) = fread(dat,[specs.record_int_len,1], 'int=>int');
                pool_data.double_data(:,i) = fread(dat,[specs.record_double_len,1], 'double=>double');
                pool_data.int_chunk_data(:,i) = fread(dat,[specs.beads*specs.record_int_chunk_len,1], 'int=>int');
                pool_data.double_chunk_data(:,i) = fread(dat,[specs.beads*specs.record_double_chunk_len,1], 'double=>double');
