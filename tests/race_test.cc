...
int main()
{
  ...
  char swbest_name[30]; sprintf(swbest_name, "swirl_best_rc%d.odr/", race_id);
    ...
    reporter rep;
    rep.init_race(proc_loc, rydat_dir, file_name, race_id);
    ...
    referee ref(nlead, npool, 12, 0.002, 1e-2, 1e-3, 1.0, 0.5, 1);
    race prace(ref,sp_min,sp_max,wl,t_phys,&rep);
    prace.init_race();
    prace.start_race(generations);
    prace.make_best_swirl(swbest_name);
